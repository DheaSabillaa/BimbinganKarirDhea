<section>
    ...
    <form method="post" action="{{ route('dokter.profile.update') }}" class="mt-6 space-y-6">
      ...
      <div class="mt-4">
        ...
        <select name="poli" id="poli" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
            @foreach($poli as $item)
                <option value="{{ $item->nama_poli }}" {{ $user->poli == $item->nama_poli ? 'selected' : '' }}>
                    {{ $item->nama_poli }}
                </option>
            @endforeach
        </select>
