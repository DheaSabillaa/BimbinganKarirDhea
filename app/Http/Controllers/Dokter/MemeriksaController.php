...
{
    $janjiPeriksa = JanjiPeriksa::with(['periksa', 'jadwalPeriksa'])->findOrFail($id);
    ...
    // Hapus semua detail periksa sebelumnya
    \App\Models\DetailPeriksa::where('id_periksa', $periksa->id)->delete();

    // Simpan ulang detail periksa (obat)
    if ($request->has('obat')) {
        foreach ($request->obat as $id_obat) {
            \App\Models\DetailPeriksa::create([
                'id_periksa' => $periksa->id,
